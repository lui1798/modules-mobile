...
const handler = {
  ...
  capturePhoto(reqData, callback, callbackFail) {
    var str =
      "hybrid://CameraPlugin:201/showCameraViewWithInfo?" +
    ...
    let options = Object.assign(
      {
        quality: 50,
        destinationType:
          window.Camera && window.Camera.DestinationType.DATA_URL, // 默认base64编码格式
  ...
  nativeCommonFun(reqData, type, callback, callbackFail) {
    try {
      if (
        window.cordova ||
        window.globalConfig.runningEnvironment === "AlipayJSBridge"
      ) {
        let type1 = "";
        let type2 = "";
        if (type && type.indexOf("_")) {
          ...
          callbackFail(
            "参数type不能为空且格式为: xxx_xxx;命名规范见 @/data/codedata/README.md"
          );
        }
        if (type1 === "Cordova") {
          console.log(
            "%c 调用原生通用模板-Cordova>>>>>>> url >>>>>> type",
            "color:purple;",
            `Cordova-${type}`
          );
