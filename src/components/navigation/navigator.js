...
export default (bus, store, moduleName, keyName) => {
  if (store) {
    store.registerModule(moduleName, {
      ...
      mutations: {
        'navigation/FORWARD': (state, { to, from, name , path }) => {
          // state.routes.push(name)
          state.routes.push(to.route.path.replace("/","") + "?" + to.route.query[keyName])
        },
  ...
  const forward = (name, toRoute, fromRoute) => {
    ...
    to.name = name
    console.log('%c navigation-forward-name', 'color:green;', name);
    console.log('%c navigation-forward-toRoute', 'color:green;', toRoute);
