<template>
  <div class="brokerageKpi-my-formal-team">
    ...
    <al-content
      class="brokerageKpi-my-formal-team-content"
      ref="insureMessage"
      :tabs="tabs"
      :isMescrollDown="true"
      :isToTop="true"
...
import { getMyFormalTeams } from "../service/getData";
import rankName from "../common/rank.json";
export default {
  name: "brokerage-kpi-my-formal-team", //使用xx-xx-xx命名方式具体看操作文档
  props: {
  ...
  mounted() {
    console.log("%c my-formal-team>生命周期>mounted", "color:green;", "");
    this.teamsData = this.brokerageKpi.myExamine;
    if (this.brokerageKpi.agentCode && this.brokerageKpi.agentCode != "") {
      this.getMyFormalTeams();
    }
  },
  watch: {},
  data() {
    return {
      title: this.$route.query["indexcalno"] + "我的团队正式考核",
      swiperTop: 52,
      ranks: rankName,
      tabs: [
        //content组件对象
  ...
  methods: {
    ...
    toHisExamine(item) {
      ...
      this.go("brokerageKpi/hisFormalExamine?hisAgentCode=" + item.agentCode + "&indexcalno=" + this.indexcalno);
    },
    getMyFormalTeams() {
      const axiosParams = {
        isLoadding: true,
      ...
      getMyFormalTeams({ data: request, config: axiosParams, method: "get" }).then(
        data => {
...
.brokerageKpi-my-formal-team {
  ...
  .brokerageKpi-my-formal-team-content {
    ::v-deep .n22-icon {
      color: #979797;
    }
    .content {
